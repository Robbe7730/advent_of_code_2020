            ...
            for (i, row) in curr_state.iter().enumerate() {
                for (j, seat_type) in row.seats.iter().enumerate() {
                    let neighbours_taken: usize = iproduct!(-1..=1, -1..=1)
                        ...
                        .filter(|(x, y)| *x >= 0 && *y >= 0 && *x < num_rows && *y < num_cols)
                        .map(|(x, y)| {
                            (curr_state[x as usize].seats[y as usize] == SeatType::Taken) as usize
                        })
                        .sum();
                    new_state[i].seats[j] = match seat_type {
                        SeatType::Floor => SeatType::Floor,
                        SeatType::Taken if neighbours_taken < 5 => SeatType::Taken,
                        SeatType::Empty if neighbours_taken == 0 => {
                            changed = true;
                            SeatType::Taken
                        }
                        SeatType::Taken => {
    ...
    fn solve_part2(&self, input: &Vec<Self::InputElement>) -> Self::Output2 {
        ...
        while changed {
            ...
            for (x, y) in iproduct!(0..num_rows, 0..num_cols) {
                ...
                visible_nw[x][y] = y > 0
                    && x > 0
                    && match curr_state[x - 1].seats[y - 1] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
                        SeatType::Floor => visible_nw[x - 1][y - 1],
                    };
                visible_n[x][y] = x > 0
                    && match curr_state[x - 1].seats[y] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
            ...
            for (x, y) in iproduct!((0..num_rows).rev(), (0..num_cols).rev()) {
                visible_e[x][y] = y < num_cols - 1
                    && match curr_state[x].seats[y + 1] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
                        SeatType::Floor => visible_e[x][y + 1],
                    };
                visible_se[x][y] = y < num_cols - 1
                    && x < num_rows - 1
                    && match curr_state[x + 1].seats[y + 1] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
                ...
                visible_s[x][y] = x < num_rows - 1
                    && match curr_state[x + 1].seats[y] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
                        SeatType::Floor => visible_s[x + 1][y],
                    };
                visible_sw[x][y] = x < num_rows - 1
                    && y > 0
                    && match curr_state[x + 1].seats[y - 1] {
                        SeatType::Taken => true,
                        SeatType::Empty => false,
            ...
            for (x, y) in iproduct!(0..num_rows, 0..num_cols) {
                ...
                new_state[x].seats[y] = match curr_state[x].seats[y] {
                    ...
                    SeatType::Empty if num_visible == 0 => {
                        changed = true;
                        SeatType::Taken
                    }
                    SeatType::Taken => {
